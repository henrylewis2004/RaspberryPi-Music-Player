...
static void dma_i2s_init(void){
	...
	for (int i = 0; i < sizeof(audio_buffer); i++){
		...
		//enabled irq0 to know when to refil
		dma_channel_set_irq0_enabled(dma_channel[i],true);
	}
	...
	if (buffer_callback){
		buffer_callback(audio_buffer[0],I2S_BUFFER_WORDS);
		buffer_callback(audio_buffer[1],I2S_BUFFER_WORDS);
	}
